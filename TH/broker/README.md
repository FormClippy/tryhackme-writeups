...
- ในไฟล์ subscribe.py เราเพิ่มโค้ดเพื่อให้ระบบรันคำสั่งตั้ง SUID บน /bin/bash:
```bash
import os
...
คำอธิบาย: คำสั่ง `chmod +s /bin/bash` จะตั้ง bit SUID ให้กับไฟล์ `/bin/bash` ทำให้เมื่อไฟล์นี้ถูกเรียกใช้งาน ผู้เรียกจะได้รับสิทธิ์ของเจ้าของไฟล์ (โดยปกติคือ root) ในขณะรันโปรแกรม ซึ่งสามารถนำไปสู่การได้มาซึ่ง shell ที่ทำงานด้วยสิทธิ์ root

- หลังจากแก้ไขไฟล์เรียบร้อย ให้สั่งรันสคริปต์ด้วยสิทธิ์ root ผ่าน sudo ตาม entry ที่เจอจาก sudo -l:
```bash
...
หากสำเร็จ ผู้ใช้งานจะได้ shell ที่รันด้วยสิทธิ์ root และสามารถตรวจสอบไฟล์หรือสั่งการใด ๆ ตามสิทธิ์ที่มี
- ต่อเนื่อง: ตรวจหาและอ่านไฟล์ root.txt
