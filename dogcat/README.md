...
🧠 วิเคราะห์:
 - ไฟล์ `backup.sh` ดูเหมือนจะทำการ backup ข้อมูลภายใน container ไปไว้ที่ path /root/container
 - สิ่งนี้ยืนยันว่า: 🐳 ระบบที่เรารันอยู่คือ Docker Container ไม่ใช่โฮสต์จริง
 - เป้าหมายต่อไป: พยายามออกจาก container ไปยัง host หรือหาทางใช้สิทธิ์ root ให้มีผลกับ host

🧨 2.17 Exploit ผ่าน backup.sh เพื่อ Escape Container

38. 🧠 วิเคราะห์สถานการณ์:
- ตอนนี้เราพบว่าไฟล์ `backup.sh` อยู่ใน path ที่เราสามารถเขียนทับได้
- ไฟล์นี้ทำงานโดยใช้คำสั่ง `tar` เพื่อ backup ข้อมูลจาก container → host
- ซึ่งหากเราฝังคำสั่ง reverse shell ลงในไฟล์นี้ แล้วรอให้สคริปต์ถูกรัน เราจะสามารถ "หลบหนีจาก Docker container" ไปยัง Host ได้

39. 🧲 รอฟังการเชื่อมต่อจาก shell ที่ย้อนกลับมา
